...
def plot(tickLoc, tickLables, residuals, sm_residuals, title, filename, save):
    plt.figure(figsize=(24, 8))
    plt.hlines(0, tickLoc[0], tickLoc[-1])
    plt.scatter(tickLoc, residuals, c = 'r', label = 'residuals')
    plt.scatter(tickLoc, sm_residuals, c = 'b', label = 'sm_residuals')
    plt.xlabel('Time')
...
def regression(y, C, ticks):
    ...
    y_array1 = np.array([[1, i] for i in y])
    sm_results = sm.OLS(C_array, y_array1).fit()
    # print(sm_results.summary())

    predictions = reg_model.predict(y_array)
    sm_predictions = sm_results.fittedvalues
    ...
    sm_residuals = []
    for i in range(len(y)):
        temp_resid = C[i] - sm_predictions[i]
        sm_residuals.append(temp_resid)
    ...
    flnm = '/regression.pdf'
    save = False
    plot(tick_loc, ticks, residuals, sm_residuals, title, flnm, save)
